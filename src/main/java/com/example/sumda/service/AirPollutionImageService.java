...
public class AirPollutionImageService {
    ...
    // 공공 API에서 데이터 가져오기
    public AirPollutionImageDto fetchFromPublicApi() throws Exception {
        ...
        // DTO 초기화
        AirPollutionImageDto airDto = new AirPollutionImageDto(); // DTO 객체 생성
        List<AirPollutionImageDto.AirPollutionImage> airPollutionImages = new ArrayList<>(); // 이미지 리스트 생성
        ...
        if (pm10response.statusCode() == 200) {
            ...
            if (itemsNode.isArray() && itemsNode.size() > 0) {
                ...
                // DTO의 이미지 리스트에 추가
                AirPollutionImageDto.AirPollutionImage airPollutionImage = new AirPollutionImageDto.AirPollutionImage();
